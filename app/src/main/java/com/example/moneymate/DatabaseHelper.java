...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void deleteExpense(String title, String category, double amount) {
        ...
        db.delete(TABLE_EXPENSE,
                COLUMN_TITLE + "=? AND " + COLUMN_CATEGORY + "=? AND " + COLUMN_AMOUNT + "=?",
                new String[]{title, category, String.valueOf(amount)});
        db.close();
    }
    public double getWeeklyTotal(){
        SQLiteDatabase db=this.getReadableDatabase();
        String query="SELECT SUM("+COLUMN_AMOUNT+") FROM "+TABLE_EXPENSE+" WHERE date>=date('now', '-6 days')";
    ...
    public double getMonthlyTotal(){
        SQLiteDatabase db=this.getReadableDatabase();
        String query="SELECT SUM("+COLUMN_AMOUNT+") FROM "+TABLE_EXPENSE+" WHERE strftime('%Y-%M', date) = strftime('%Y-%m', 'now')";
        Cursor cursor= db.rawQuery(query,null);
        double total=0;
        if(cursor.moveToFirst()){
            total=cursor.getDouble(0);
